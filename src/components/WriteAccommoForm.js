...
const WriteAccommoForm = ({ onSubmit }) => {
  ...
  const pickImages = async () => {
    ...
    if (!result.canceled) {
      const selectedImages = result.assets.map((asset) => {
        ...
        return {
          uri: asset.uri,
          name: sanitizeFileName(asset.fileName || `image_${Date.now()}`), // 파일 이름 정리 추가
          type: fileType,
        };
      });
  ...
  const handleDateSelection = (day) => {
    ...
    setActiveDateField(null); // 활성 날짜 필드 리셋
  };

  // 날짜 버튼 클릭 처리
  const toggleCalendar = (field) => {
    if (activeDateField === field) {
      setShowCalendar(false); // 이미 선택된 필드 클릭 시 달력 닫기
      setActiveDateField(null); // 활성 필드 리셋
    } else {
      setActiveDateField(field); // 클릭한 필드를 활성화
      setShowCalendar(true); // 달력 열기
    }
  };
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          <View style={styles.dateContainer}>
            <Text style={styles.dateText}>가능한 날짜 :</Text>
            <Pressable
          ...
          {showCalendar && (
            <Calendar
              ...
              markedDates={{
                [startDate]: { selected: true, color: 'blue' },
                [endDate]: { selected: true, color: 'red' },
              }}
            />
          )}
          <Input
...
export default WriteAccommoForm;
